...
class Cubist(BaseEstimator, RegressorMixin):
    ...
    n_rules : int, default=500
        Limit of the number of rules Cubist will build. Recommended value is 500.

    n_committees : int, default=5
        Number of committees to construct. Each committee is a rule based model and beyond the first tries to correct 
        the prediction errors of the prior constructed model. Recommended value is 5.
    ...
    def fit(self, X, y):
        ...
        self.names_string_ = make_names_string(X, w=self.weights, label=self.target_label)
        self.data_string_ = make_data_string(X, y, w=self.weights)
